...
| `committedDay` | `0..5` | 확정 Day 기록, 새 게임 `0` |
...
이 지속은 현재 run 안의 Day 1~5 전환을 뜻한다. 디스크 세이브·로드, 메타 영구 해금과 환불용 비용 snapshot은 이번 범위 밖이다.
...
### 5.3 늦은 이중 성장: Day 3 신호 부족 뒤 복구 여지
...
| Day 3 정산 | 구조 미완료 | 0 | 1 | 1 | 2 | Day 4 진입, `Result=None` |

판정: 모듈 비용은 성공 자원을 없애는 함정이 아니지만, 같은 run에 성장 투자를 늦게 하나 더 사면 구조 비용과 실제로 충돌한다. 5일제에서는 Day 4~5에 W를 보충해 복구할 수 있고 Day 5 종료 때까지 못 고친 경우만 기한 실패다. 실패한 신호 시도나 모듈 재확인에서는 추가 차감이 없어야 한다.
