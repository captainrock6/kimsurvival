...
배치 플레이테스트에는 일반 설비의 실제 접근·사용 여부를 별도로 기록한다. O2 리베이스 구현은 자유 배치를 폐기하고 방별 고정 설치 지점, 직접 접근 팝업, 지붕 있는 시작층·2층·지하실, 침대·소파를 통합했다. 자동 공간 계약은 PASS지만, O3에서 사람 입력으로 설치·재배치·직접 사용과 작업대 상호작용 우선순위를 확인해야 한다.
