...
| 베이스캠프 제한적 자유 배치 | NOT RUN | 후속 확정 범위. 기준 커밋에는 배치/재배치 시스템이 없어 별도 테스트 계획으로 이관 |
| 한국어 기본/영어/로케일 확장 | NOT RUN | 후속 확정 범위. 기준 커밋에는 로케일 서비스·ko/en 테이블·선택 저장·폴백 로그가 없고 플레이어 노출 문구가 한국어 리터럴로 연결됨 |
...
## 후속 확정 범위: 베이스캠프 제한적 자유 배치

2026-08-22 메인 디렉터에서 베이스캠프 제한적 자유 배치가 QA 기준에 추가되었다. `2a6e9e6`은 모닥불·작업대·빗물받이·구조 신호대가 고정 좌표로 그려지며 배치 유령, 재배치, 경계/겹침/통로 판정, 특수 앵커 시스템이 없어 이 기능은 본 감사에서 실행하지 않았다. 따라서 위 기존 판정과 증거는 그대로 유지하고 새 항목을 **NOT RUN**으로 기록한다.

구현 후에는 [베이스캠프 제한적 자유 배치 QA 체크리스트 및 테스트 계획](base-camp-limited-placement-test-plan.md)을 필수 게이트로 적용한다. 일반 설비 최소 2종의 배치·재배치, 구조 신호대 전용 앵커, 유효/무효 유령, 경계·겹침·출입구·필수 통로 거부, 자원/인스턴스 불변식, 상호작용 접근, 키보드·마우스/실제 게임패드 동등성, 1280×800 가독성을 별도 증거로 검증한다. 완전 자유 회전·물리 적층·복층 건축은 현재 제외 범위다.

## 후속 확정 범위: 한국어 기본·영어 지원 및 로케일 확장

2026-08-22 메인 디렉터에서 한국어 기본, 영어 지원과 향후 로케일 확장 구조가 QA 기준에 추가되었다. `2a6e9e6`에는 로케일 서비스/번역 카탈로그/선택 저장/한국어 폴백 로그가 없고 HUD·월드 라벨·세션 메시지·제품명이 한국어 리터럴로 직접 연결되어 있어 이 기능을 본 감사에서 실행하지 않았다. 따라서 기존 PASS/FAIL과 증거는 그대로 유지하고 신규 항목을 **NOT RUN**으로 기록한다.

구현 후에는 [한국어 기본·영어 지원 및 로케일 확장 QA 체크리스트/테스트 계획](ko-en-localization-test-plan.md)을 필수 게이트로 적용한다. ko/en 즉시 전환과 재실행 유지, 누락 번역의 한국어 폴백/구조화 로그, 포맷 변수와 복수·수량, 한글·라틴·확장 글리프, 1280×800 영어/가짜 장문 로케일, 키보드·마우스/실제 게임패드 설정 조작, 한국어와 영어 각각의 전체 핵심 루프, 승인된 디버그 allowlist 밖의 플레이어 노출 한·영 하드코딩 문자열 0건을 새 실행 증거로 검증한다. 실제 게임패드가 없으면 미검증이며 PASS로 승격하지 않는다. 공식 영문 게임 제목은 미정(TBD)으로 취급하고 임시명은 데이터 경로와 레이아웃만 검증한다.
